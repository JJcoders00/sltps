...
async function loadStaffDashboard() {
    ...
    try {
        ...
        querySnapshot.forEach((doc) => {
            ...
            const docsHtml = `
                ${data.documents?.birthCertificate ? `<a href="${data.documents.birthCertificate}" target="_blank" style="color:var(--primary-accent); margin-right: 5px;">Birth Cert</a>` : ''}
                ${data.documents?.parentId ? `<a href="${data.documents.parentId}" target="_blank" style="color:var(--primary-accent); margin-right: 5px;">ID</a>` : ''}
                ${data.documents?.studentPhoto ? `<a href="${data.documents.studentPhoto}" target="_blank" style="color:var(--primary-accent);">Photo</a>` : ''}
            `;
            ...
            html += `
                <tr style="border-bottom: 1px solid rgba(255,255,255,0.1);">
                    ...
                    <td style="padding: 1rem; font-size: 0.85rem;">${docsHtml || 'None'}</td>
                </tr>
            `;
        });
...
async function uploadDocument(fileEl, trackingId, docType) {
    const file = fileEl.files[0];
    if (!file) return null;
    
    const fileRef = ref(storage, `admissions/${trackingId}/${docType}_${file.name}`);
...
document.getElementById('application-form')?.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        // 1. Upload Documents
        const birthUrl = await uploadDocument(document.getElementById('doc-birth'), trackingId, 'birth_cert');
        const idUrl = await uploadDocument(document.getElementById('doc-id'), trackingId, 'parent_id');
        const photoUrl = await uploadDocument(document.getElementById('doc-photo'), trackingId, 'photo');
        ...
        const applicationData = {
            ...
            parent: {
                ...
                mobile: document.getElementById('par-mobile').value,
